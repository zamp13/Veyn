...
./parsemeCuptToDimsumWithVMWE.py ${OPT_CUPT} ${DATA}${LANG}${DEV}${CUPT} > ${DATA}${LANG}${DEV}${DIMSUM}
...
./RNNMultiGRUWithVMWE.py ${ARGUMENTS}  > ${RESULT}${LANG}${PREDICT}${DEV}${ind}${TEXT}
echo "End train "${ind}
done

python ../voteMajoritaire ${RESULT}${LANG} > ${RESULT}${LANG}${PREDICT}${DEV}${PRED}

./addPredictToDimsumWithVMWE.py ${OPT_DIMSUM} ${DATA}${LANG}${DEV}${DIMSUM} ${OPT_TAG} ${RESULT}${LANG}${PREDICT}${DEV}${PRED} > ${DATA}${LANG}${PREDICT}${DEV}${DIMSUM}
