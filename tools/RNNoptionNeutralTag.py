...
longopts = ["ignoreColumns=", "columnOfTags=", "test=", "train=", "embeddings=", "neutralTags="]
shortopts = "i:t:e:n:"
filenameTrain = None
filenameTest = None
...
-n or --neutralTags <n-entities>
    To give tag(s) corresponding to 'neutral' tag.
    The RNN will put a smaller weight to these tags.
...
def treat_options( opts, arg, n_arg, usage_string ) :
    ...
    for o, a in ctxinfo.iter(opts):
        ...
        elif o in ("--neutralTags", "-n"):
            neutralTags = a.split(":")
            for i in range(len(neutralTags)):
                neutralTags[i] = neutralTags[i]
        else:
...
def vectorize(features, tags, vocab, unroll):
    ...
    for i in range(len(tags)):
        for j in range(unroll):
            ...
            if(curTag in codeNeutralTags):
                sample_weight[i][j] = 0.1
            else:
...
def main():
    ...
    for tag in neutralTags:
        codeNeutralTags.append(vocab[numColTag][tag])
    ...
    if(neutralTags == []):
        model.fit(X_train, Y_train, batch_size=batch, epochs=epochs, shuffle=True, validation_data=(X_test, Y_test))
    else:
